...
public class PuzzleState implements State {
    ...
    */
    ...
    public static int tilesOutOfPlace(State current, State goal) {
        int count = 0;
        PuzzleState curState = (PuzzleState) current;
        PuzzleState goalState = (PuzzleState) goal;
        int[][] curTiles = curState.getTiles();
        int[][] goalTiles = goalState.getTiles();
        int rows = curTiles.length;
        int columns = curTiles[0].length;
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < columns; c++) {
                if (curTiles[r][c] != 0 && curTiles[r][c] != goalTiles[r][c]) {
                    count++;
                }
            }
        }
    ...
    public static int sumManhattan(State current, State goal) {
        ...
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < columns; c++) {
                if (curTiles[r][c] != 0 && curTiles[r][c] != goalTiles[r][c]) {
                    loc = goalState.getLocation(curTiles[r][c]);
                    sum += Math.abs(loc.getRow() - r);
                    sum += Math.abs(loc.getColumn() - c);
                }
            }
        }
        return sum;
